...
import com.jollywhoppers.atproto.ClientAtProtoClient
import com.jollywhoppers.atproto.ClientAtProtoCommands
import com.jollywhoppers.atproto.ClientSessionManager
...
object AtprotoconnectClient : ClientModInitializer {
	...
	override fun onInitializeClient() {
		...
		try {
			...
			logger.info("  ✓ Client-side authentication")
			logger.info("  ✓ Passwords never sent to server")
			logger.info("  ✓ Local session storage")
			...
			context.client().execute {
				if (packet.success) {
					Minecraft.getInstance().gui.chat.addMessage(
						Component.literal("§a✓ Server confirmed authentication!")
					...
					Minecraft.getInstance().gui.chat.addMessage(
						Component.literal("§c✗ Server rejected authentication")
							.append(Component.literal("\n§7${packet.message}"))
					)
